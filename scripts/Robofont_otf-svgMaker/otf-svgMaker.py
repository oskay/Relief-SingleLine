# 2021
# by Frederik Berlaen for isdaT-type 
# (institut supérieur des arts et du design de Toulouse: 
# fonts and type design tools, project coordination by François Chastanet, 
# Relief CNC central-line / single-stroke font)
...
for glyph in font:
    ...
    pen = OutlinePen(font, offset=lineThickness, connection="round", cap="round", optimizeCurve=True)
...
def getSVGDocForGlyph(glyph, gid):
    ...
    svgPath = f'<path stroke="{strokeColor}" fill="none" stroke-width="{lineThickness}" d="{svgPen.getCommands()}" />'
